    ...
    <div class="row page-titles mx-0 mb-4">
        <div class="col-sm-6 p-md-0">
            <div class="welcome-text">
                <h4 class="text-primary fw-bold">📝 Editar Pedido</h4>
                <p class="mb-0">Modifica los datos de un pedido existente</p>
            </div>
        </div>
            ...
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-header bg-secondary text-white rounded-top-4">
                    <h4 class="card-title mb-0 fw-semibold text-white">
                        <i class="fa fa-pen-to-square me-2"></i> Actualizar Pedido
                    </h4>
                </div>
                ...
                <div class="card-body p-5" style="background-color: #b3b3b3; border-radius: 0 0 1rem 1rem;">
                    <form method="POST" action="{{ route('pedidos.update', $pedido->id) }}">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select name="tipo" id="tipo" class="form-control shadow-sm border rounded-3 p-0" required>
                                    <option value="mesa" {{ old('tipo', $pedido->tipo) == 'mesa' ? 'selected' : '' }}>Para la mesa</option>
                                    <option value="llevar" {{ old('tipo', $pedido->tipo) == 'llevar' ? 'selected' : '' }}>Para llevar</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="nombre" id="nombre"
                                       ...
                                       value="{{ old('nombre', $pedido->nombre) }}" placeholder="Ej: Juan Pérez">
                            </div>

                            <!-- Mesa -->
                            <div class="form-group col-md-4">
                                <label for="mesa_id" class="form-label fw-semibold text-black">Mesa</label>
                                <select name="mesa_id" id="mesa_id" class="form-control shadow-sm border rounded-3 p-0">
                        ...
                        <!-- 🧾 Detalles del Pedido -->
                        <h5 class="fw-bold text-black mb-3"><i class="fa fa-utensils me-2"></i> Editar Detalles del Pedido</h5>
                            ...
                            <table class="table table-bordered align-middle text-center" id="detalles-table">
                                ...
                                <tbody>
                                    @foreach ($pedido->detalles as $i => $detalle)
                                        <tr>
                                            <td>
                                                <select name="detalles[{{ $i }}][plato_id]" class="form-control plato-select shadow-sm border rounded-3" required>
                                                    <option value="">Seleccione un plato</option>
                                                    @foreach ($platos as $plato)
                                                        <option value="{{ $plato->id }}"
                                                            data-precio="{{ $plato->precio }}"
                                                            {{ $detalle->plato_id == $plato->id ? 'selected' : '' }}>
                                                            {{ $plato->nombre }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="number" name="detalles[{{ $i }}][cantidad]" class="form-control cantidad-input shadow-sm border rounded-3"
                                                    min="1" value="{{ $detalle->cantidad }}" required>
                                            </td>
                                            <td>
                                                <input type="number" step="0.01" name="detalles[{{ $i }}][precio]" class="form-control precio-input shadow-sm border rounded-3"
                                                    min="0" value="{{ $detalle->precio }}" readonly>
                                            </td>
                                            <td>
                                                <input type="number" step="0.01" name="detalles[{{ $i }}][subtotal]" class="form-control subtotal-input shadow-sm border rounded-3"
                                                    value="{{ $detalle->cantidad * $detalle->precio }}" readonly>
                                            </td>
                                            <td>
                        ...
                        <div class="text-end mt-4">
                            <h5 class="fw-bold text-black">Total: Bs. <span id="total-final">{{ number_format($pedido->total, 2) }}</span></h5>
                        </div>
...
<script>
    ...
    // ⚙️ Ejecutar esto al cargar la página para las filas existentes
    Array.from(detallesTable.rows).forEach(row => {
        agregarEventosFila(row);
        actualizarFila(row); // <-- recalcula el precio y subtotal inicial
    });
